...
if __name__ == '__main__':
    ...
    column_name = ['commitTime', 'ns', 'nd', 'nf', 'entrophy', 'la', 'ld', 'lt', 'fix', 'ndev', 'age', 'nuc', 'exp',
                   'rexp', 'sexp', 'bug']
    model_names = ['naive_bayes', 'support_vector_machine', 'gradient_boosting', 'decision_tree',
    ...
    for model_name in model_names:
        for i in range(0, pro_num):
            ...
            for fold in range(len(train_folds)):
                train_data, train_label, test_data, test_label, gap_data, gap_label, LOC = time_wise_fold_divided(fold,
                                                                                                                  train_folds,
                                                                                                                  test_folds,
                                                                                                                  gap_folds)

                # ensure train data: the number of defect > non defect, only one class
                if len(np.unique(train_label)) < 2 or list(train_label).count(1) < 6 or list(train_label).count(
                        1) > list(train_label).count(0):
                    continue
                for method, sampler in sampling_methods.items():
                    ...
                    if (model_name in ['naive_bayes', 'knn']):
                        ...
                        explainer_shap = shap.Explainer(model,
                                                        check_additivity=False)  # (model_name in ['gradient_boosting','decision_tree']):
